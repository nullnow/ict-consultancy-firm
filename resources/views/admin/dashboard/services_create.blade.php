    ...
    <div class="bg-dash-surface p-8 rounded-xl space-y-6">
        ...
        <form action="{{ route('admin.services.store') }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" name="subtitle" value="{{ old('subtitle') }}" placeholder="e.g., Reach instantly. Engage intelligently." class="w-full p-4 bg-dash-bg border border-white/10 rounded text-white text-sm focus:outline-none focus:border-dash-accent-blue-light">
            ...
            <div>
                <label class="block text-xs uppercase tracking-wider text-dash-muted font-bold mb-2">Marketplace Structural Positioning Paragraph (Intro Text — Optional)</label>
                <textarea name="intro_text" rows="5" placeholder="Describe the structural operational problem this platform system solves for the Tanzanian market..." class="w-full p-4 bg-dash-bg border border-white/10 rounded text-white text-sm focus:outline-none focus:border-dash-accent-blue-light">{{ old('intro_text') }}</textarea>
